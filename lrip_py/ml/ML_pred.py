...
def ml_save(kres_path, expt_path, model_lis):
   ...
   for mol in model_lis:
      ...
      tau.append(tau_tmp.statistic)

   # Summarize trained model performance
   rmse     = [round(val, 3) for val in rmse]
   mae      = [round(val, 3) for val in mae]
   corr     = [round(val, 3) for val in corr]
   corr2    = [round(val, 3) for val in corr2]
   p_value  = [round(val, 3) for val in p_value]
   pi       = [round(val, 3) for val in pi]
   tau      = [round(val, 3) for val in tau]
   value = {'RMSE':rmse, 'MAE':mae, 'CORR':corr, 'CORR2':corr2, 'p-value':p_value, 'PI':pi, 'TAU':tau}
