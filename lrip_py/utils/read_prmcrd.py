...
def read_prmcrd(crd_path, log_file=None):
    if os.path.exists(crd_path):
        pass
    else:
        _write_log(log_file, 'ERROR: the prmcrd file does not exist: %s'%crd_path)
        raise SystemExit
    prmcrd = {
        'name':'',
        'num_atom':0,
    ...
    with open(crd_path, 'r') as crd_file:
        ...
        if num_atom == prmcrd['num_atom']:
            ...
            _write_log(log_file, 'ERROR: prmcrd file inconsistant, number of atoms: %s'%crd_path)
            raise SystemExit
    return(prmcrd)
