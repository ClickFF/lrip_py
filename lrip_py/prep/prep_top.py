...
def prep_top(ligs, lig_dir_path, receptor_path, tleap_template_path, log_file=None):
...
def prep_top_parallel(lig, lig_dir_path, receptor_path, tleap_template_path, log_file=None):

    """
    Prepare AMBER topology and coordinate files for each ligand in the provided directory.

    Parameters:
    - ligs: Dictionary of ligands with their IDs and net charges.
    - lig_dir_path: Path to the directory containing ligand files.
    - receptor_path: Path to the receptor PDB file.
    - tleap_template_path: Path to the template file for tleap input.
    - log_file: Optional path to a log file for logging output.
    ...
    with tempfile.TemporaryDirectory() as temp_dir: 
        ...
        try:
            ...
            _write_log(log_file, f"Error processing ligand {lig}: {e}")
            _write_log(log_file, f"Command output: {e.stdout.decode()}")
            _write_log(log_file, f"Command error: {e.stderr.decode()}")
